...
/**
 * flush-data.js — Clear stored data from the n8netwatch SQLite database.
 *
 * Usage:
...
const noDataFlag = !args.includes('--alerts') && !args.includes('--all') && !args.includes('--ping-results');
const flushPingResults  = flushAll || args.includes('--ping-results') || noDataFlag;
...
function run() {
  ...
  try {
    ...
    console.error('ERROR: better-sqlite3 is not installed. Run "npm install" first.');
    process.exit(1);
  }

  if (!require('fs').existsSync(dbPath)) {
    console.error(`ERROR: Database not found at ${dbPath}`);
    console.error('       Run the application at least once to create the database.');
    process.exit(1);
  }
  ...
  console.log('');
  console.log('n8netwatch — Flush Database Data');
  console.log('=================================');
  ...
  function flush() {
    ...
    const tx = db.transaction(() => {
      let pings = 0, alerts = 0, targets = 0;
    ...
    console.log('Done!');
    if (flushPingResults) console.log(`  Deleted ${result.pings} ping result(s)`);
  ...
  rl.question(`Are you sure you want to delete ${summary.join(', ')}? [y/N]: `, (answer) => {
    rl.close();
