...
function drawLineChart(doc, values, x, y, w, h, label, color = [59, 130, 246]) {
  const valid = values.filter(v => v !== null && v !== undefined && !isNaN(Number(v)));
  if (valid.length < 2) {
    doc.setFontSize(8);
...
export function generatePDFReport(reportData) {
  const { target, uptime, metrics, ping_results, alerts, generated_at } = reportData;
  ...
  doc.text(`Generated: ${new Date(generated_at).toLocaleString()}`, pageW - margin, 12, { align: 'right' });

  const isUp = target.is_alive;
  doc.setFillColor(...(isUp ? [34, 197, 94] : isUp === 0 ? [239, 68, 68] : [107, 114, 128]));
  doc.circle(pageW - margin - 3, 21, 2.5, 'F');
  doc.setFontSize(8);
  doc.setTextColor(255, 255, 255);
  doc.text(isUp ? 'UP' : isUp === 0 ? 'DOWN' : 'UNKNOWN', pageW - margin - 7, 21.5);
  ...
  autoTable(doc, {
    ...
    head: [['Metric', 'Value']],
    body: [
      ['Avg Latency',  target.avg_latency !== null && target.avg_latency !== undefined ? `${fmt(target.avg_latency)} ms` : 'N/A'],
  ...
  if (metrics && metrics.length > 0) {
    ...
    drawLineChart(doc, latencyValues, margin,             curY, chartW, chartH, 'Avg Latency (ms)', [59, 130, 246]);
    drawLineChart(doc, lossValues,    margin + chartW + 6, curY, chartW, chartH, 'Packet Loss (%)',  [239, 68, 68]);
  ...
  if (ping_results && ping_results.length > 0) {
    ...
    const pingRows = ping_results.slice(0, 50).map(r => [
    ...
    autoTable(doc, {
      ...
      body: pingRows,
      styles:     { fontSize: 7, textColor: [226, 232, 240], fillColor: [30, 41, 59] },
      headStyles: { fillColor: [15, 23, 42], textColor: [148, 163, 184], fontStyle: 'bold' },
      alternateRowStyles: { fillColor: [30, 41, 59] },
      tableLineColor: [55, 65, 81],
      tableLineWidth: 0.2,
      didParseCell: (data) => {
        if (data.column.index === 1 && data.section === 'body') {
          data.cell.styles.textColor = data.cell.raw === 'UP' ? [34, 197, 94] : [239, 68, 68];
        }
      },
    });
    ...
    autoTable(doc, {
      ...
      didParseCell: (data) => {
        ...
        if (data.column.index === 3 && data.section === 'body') {
          data.cell.styles.textColor = data.cell.raw === 'Active' ? [239, 68, 68] : [34, 197, 94];
        }
      },
    });
  }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setTextColor(100, 116, 139);
    doc.text(
      `n8netwatch · Page ${i} of ${totalPages}`,
      pageW / 2,
...
export function generateCSVReport(reportData) {
  ...
  if (ping_results && ping_results.length > 0) {
    ...
    lines.push('Time,Status,Avg Latency (ms),Min Latency (ms),Max Latency (ms),Jitter (ms),Packet Loss (%),Packets Sent,Packets Received');
